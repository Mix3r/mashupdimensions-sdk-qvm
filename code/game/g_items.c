...
void Add_Ammo (gentity_t *ent, int weapon, int count, int nAmmoLimit)
{
        // Mix3r_Durachok: set weapon-dependent maximum
        if (nAmmoLimit == 0) {
                nAmmoLimit = 200;
        }
	ent->client->ps.ammo[weapon] += count;
	if (ent->client->ps.ammo[weapon] > nAmmoLimit) {
		ent->client->ps.ammo[weapon] = nAmmoLimit;
	}
}
...
int Pickup_Ammo (gentity_t *ent, gentity_t *other)
{
	...
	Add_Ammo( other, ent->item->giTag, quantity, atoi(ent->item->precaches) );
...
int Pickup_Weapon (gentity_t *ent, gentity_t *other)
{
	...
	if ( ent->count < 0 ) {
		quantity = 0; // None for you, sir!
                ...
                // also special case for Airhog vehicle to lock player to "mounted" chaingun only
                        ...
                        other->client->ps.weapon = WP_CHAINGUN;
                        quantity = -ent->count;
                }
	} else {
		if ( ent->count ) {
		...
		//if ( ! (ent->flags & FL_DROPPED_ITEM) && g_gametype.integer != GT_TEAM ) {
                if ((ent->flags & FL_DROPPED_ITEM) || (g_gametype.integer == GT_TEAM && g_weaponTeamRespawn.integer != g_weaponRespawn.integer)) {
                } else {
			// respawning rules
	...
	if (ent->item->giTag == WP_GRAPPLING_HOOK) {
                // Mix3r_Durachok: special case for mission mode to use this entity as mission pickup instead of grapple pickup
                // set entity property count -1 in map editor to prevent getting grapple
                if ( ent->count < 0 ) {
...
void Touch_Item (gentity_t *ent, gentity_t *other, trace_t *trace)
{
	...
	//instant gib
        if (ent->item->giTag == WP_GRAPPLING_HOOK || ent->item->giTag == PW_FLIGHT) {
	} else if ((g_instantgib.integer || g_rockets.integer || g_gametype.integer == GT_CTF_ELIMINATION || g_elimination_allgametypes.integer)
...
void ClearRegisteredItems( void )
{
	...
	if( g_gametype.integer == GT_HARVESTER ) {
		RegisterItem( BG_FindItem( "item_redcube" ) );
		RegisterItem( BG_FindItem( "item_bluecube" ) );
	}

	if(g_gametype.integer == GT_DOUBLE_D ) {
		RegisterItem( BG_FindItem( "team_DD_pointAblue" ) );
		RegisterItem( BG_FindItem( "team_DD_pointAred" ) );
		RegisterItem( BG_FindItem( "team_DD_pointAwhite" ) );
		RegisterItem( BG_FindItem( "team_DD_pointBblue" ) );
		RegisterItem( BG_FindItem( "team_DD_pointBred" ) );
		RegisterItem( BG_FindItem( "team_DD_pointBwhite" ) );
	}

	if(g_gametype.integer == GT_DOMINATION ) {
		RegisterItem( BG_FindItem( "team_dom_pointWhite" ) );
		RegisterItem( BG_FindItem( "team_dom_pointRed" ) );
		RegisterItem( BG_FindItem( "team_dom_pointBlue" ) );
	}

	if (g_gametype.integer == GT_POSSESSION) {
		RegisterItem(BG_FindItem("team_CTF_neutralflag") );
	}
...
void G_SpawnItem (gentity_t *ent, gitem_t *item)
{
	...
	ent->physicsBounce = 0.50;		// items are bouncy
        ...
        if (item->giTag == WP_GRAPPLING_HOOK) {
        } else if (g_gametype.integer == GT_ELIMINATION || g_gametype.integer == GT_LMS ||
...
void G_RunItem( gentity_t *ent )
{
	...
	if ( contents & CONTENTS_NODROP ) {
		if (ent->item && ent->item->giType == IT_TEAM && (Q_strequal(ent->item->classname,"item_redcube") || Q_strequal(ent->item->classname,"item_bluecube"))) {
