	...
	if( level.numNonSpectatorClients<3 && score < 0 && !G_IsATeamGametype(g_gametype.integer)) {
		for ( i = 0 ; i < level.maxclients ; i++ ) {
			...
			ScorePlum(ent, origin, -score, 0);
                ...
                if (bot_spcollab && (g_entities[ent->s.number].r.svFlags & SVF_BOT)) {
                        // Mix3r_Durachok: let's add score to all collaboration bots, or to players if punish (score > 1000)
                        for ( i = 0 ; i < level.maxclients ; i++ ) {
                                if ( level.clients[i].sess.sessionTeam != TEAM_SPECTATOR ) {
                                        if ( level.clients[i].pers.connected == CON_CONNECTED ) {
                                                if (score > 1000) {
                                                        if (!(g_entities[i].r.svFlags & SVF_BOT)) {
                                                                level.clients[i].ps.persistant[PERS_SCORE] += score-1000;
                                                        }
                                                } else {
                                                        if ((g_entities[i].r.svFlags & SVF_BOT)) {
                                                                level.clients[i].ps.persistant[PERS_SCORE] += score;
...
void TossClientItems( gentity_t *self )
{
	...
	if ( g_gametype.integer == GT_TEAM && g_weaponTeamRespawn.integer != g_weaponRespawn.integer ) {
        } else {
		angle = 45;
...
void TossClientCubes( gentity_t *self )
{
	...
	if( self->client->sess.sessionTeam == TEAM_RED ) {
		item = BG_FindItem( "item_redcube" );
	}
	else {
		item = BG_FindItem( "item_bluecube" );
	}
	...
	if (attacker && attacker->client) {
		...
		if ( attacker == self || OnSameTeam (self, attacker ) ) {
			...
			if (g_gametype.integer!=GT_LMS && g_gametype.integer != GT_POSSESSION) {
                                if (bot_spcollab && (attacker->r.svFlags & SVF_BOT) && (self->r.svFlags & SVF_BOT)) {
                                        // Mix3r_Durachok: collaborationists fragged comrade, punish 'em
                                        AddScore( attacker, self->r.currentOrigin, 1001 );
                                } else {
				        AddScore( attacker, self->r.currentOrigin, 1 );
                                }
			}
...
void G_Damage( gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, vec3_t dir, vec3_t point, int damage, int dflags, int mod )
{
	...
	if ( targ->s.eType == ET_MOVER ) {
		if ( targ->use && targ->moverState == MOVER_POS1 ) {
                        // Mix3r_Durachok: they actually do for now, if health property is negative :)
                        if (targ->health < 0) {
                                if (targ->damage == 0) {
                                        targ->damage = targ->health;
                                }

                                targ->health += damage;
                                if (targ->health < 0) {
                                        impactpoint[0] = targ->damage;
                                        impactpoint[0] = targ->health / impactpoint[0];
                                        //G_Printf( "dmg:%.2f\n", impactpoint[0] );
                                        if (impactpoint[0] > 0.6666) {
                                                asave=9;
                                        } else if (impactpoint[0] > 0.3333) {
                                                asave=8;
                                        } else {
                                                asave=7;
                                        }
                                        if (mod == MOD_SHOTGUN) {
                                                targ->sumImpact_dmg += damage;
                                        } else {
                                                if (mod == MOD_GAUNTLET) {
                                                        //add 100 to flag to make client play punch sound
                                                        asave += 100;
                                                }
                                ...
                                if (targ->target) {
                                        gentity_t *dpre = NULL;
                                        dpre = G_Find (dpre, FOFS(targetname), va("pre_%s",targ->target));
                                        if (dpre) {
                                                dpre->use( dpre, inflictor, attacker );
                                                asave = 1;
                                        }
                                }
...
//So people can be telefragged!
	...
	// asave = CheckArmor (targ, damage, dflags);
        ...
        if (dflags & DAMAGE_NO_ARMOR) {
                ...
                max = client->ps.stats[STAT_ARMOR];
                asave = ceil( damage * ARMOR_PROTECTION );
                if (asave >= max) asave = max;
                client->ps.stats[STAT_ARMOR] -= asave;
        }
	...
	if (damage) {
		...
		if ( targ->client ) {
			targ->client->ps.stats[STAT_HEALTH] = targ->health;
                        if (targ->health + damage > 0 && targ != attacker) {
                                if (mod == MOD_SHOTGUN) {
                                        targ->sumImpact_dmg += damage;
                                } else {
                                        if (targ->client->ps.stats[STAT_ARMOR] > targ->client->ps.stats[STAT_MAX_HEALTH]*0.5) {
                                                asave = 3;
                                        } else if (asave > 0 || targ->health >= targ->client->ps.stats[STAT_MAX_HEALTH]) {
                                                asave = 2;
                                        } else {
                                                asave = 1;
                                        }
