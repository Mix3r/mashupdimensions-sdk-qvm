...
void BotUseKamikaze(bot_state_t *bs) {
	...
	//
	BotVisibleTeamMatesAndEnemies(bs, &teammates, &enemies, KAMIKAZE_DIST*0.5);
        //BotAI_Print(PRT_MESSAGE, "teammates = %d\n", teammates);
        //BotAI_Print(PRT_MESSAGE, "enemies = %d\n", enemies);
	//
        if (enemies > 0 && enemies > teammates) {
		trap_EA_Use(bs->client);
		return;
	}
}
...
int BotSameTeam(bot_state_t *bs, int entnum) {
	...
	if (entnum < 0 || entnum >= MAX_CLIENTS) {
        ...
        if (gametype==GT_SINGLE_PLAYER) {
           //if (g_entities[entnum].r.svFlags & SVF_BOT) {
           //   if ((int)trap_Cvar_VariableValue( "g_AllowVote" ) == 2) {  // dirty dirty collab check
           //      return qtrue;
           //   }
           //}
           if (bot_spcollab) {
