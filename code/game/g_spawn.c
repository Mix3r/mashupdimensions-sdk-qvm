...
void G_SpawnGEntityFromSpawnVars( void ) {
	...
	if ( g_gametype.integer == GT_SINGLE_PLAYER || G_IsACoop()) {
	...
	if (G_IsATeamGametype(g_gametype.integer) && !G_IsACoop()) {
...
void SP_worldspawn( void ) {
	char	     *s;
        fileHandle_t f_mus;
        int          i_mus;
        char	     map_name[MAX_CVAR_VALUE_STRING];
	...
	if ( *g_music.string && !Q_strequal( g_music.string, "none" ) ) {
		s = va("%s",g_music.string);
	} else {
                trap_Cvar_VariableStringBuffer("mapname",map_name,sizeof(map_name));
                s = va("music/%s.ogg",map_name);
                i_mus = trap_FS_FOpenFile(s, &f_mus, FS_READ);
                if (i_mus > 0) {
                   trap_FS_FCloseFile(f_mus);
                } else {
                   s = va("music/%s.wav",map_name);
                   i_mus = trap_FS_FOpenFile(s, &f_mus, FS_READ);
                   if (i_mus > 0) {
                      trap_FS_FCloseFile(f_mus);
                   }
