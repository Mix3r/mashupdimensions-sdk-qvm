...
static gentity_t *SelectAnyTeamSpawnPoint( int filter_flags ) {
	gentity_t	*spot;
	int			count;
	int			selection;
	gentity_t	*spots[MAX_SPAWN_POINTS];
	...
	while ((spot = G_Find (spot, FOFS(classname), c)) != NULL) {
		if (spot->flags & filter_flags) {
			continue;
		}
		last_valid_spot = spot;
...
gentity_t *SelectSpawnPoint ( vec3_t avoidPoint, vec3_t origin, vec3_t angles, int filter_flags ) {	
	...
	if ( spot == nearestSpot ) {
		...
		spot = SelectRandomDeathmatchSpawnPoint ( filter_flags );
		if ( spot == nearestSpot ) {
			// last try     Mix3r -- try team spots now
                        spot = SelectAnyTeamSpawnPoint ( filter_flags );
                        if ( spot == NULL ) {
			   spot = nearestSpot;
                        }
		}		
...
//unlagged - client options
	...
	// team task (0 = none, 1 = offence, 2 = defence)
        // Mix3r_Durachok: let's set teamtask to 9 for coop mode (to undress team colors in client)
        if ( G_IsACoop() ) {
                teamTask = 9;
        } else {
...
//	client->areabits = savedAreaBits;
	...
	if (!G_IsARoundBasedGametype(g_gametype.integer) && !g_elimination_allgametypes.integer) {
		...
		ent->health = client->ps.stats[STAT_HEALTH] = client->ps.stats[STAT_MAX_HEALTH] + 25;
                // Mix3r_Durachok: set time residual to correct value to delay countdown 1 second, so player don't lose 1 hp immediately
                client->timeResidual = level.time + 1000;
	} else {
		client->ps.stats[STAT_WEAPONS] |= ( 1 << WP_GAUNTLET );
		client->ps.ammo[WP_GAUNTLET] = -1;
		client->ps.ammo[WP_GRAPPLING_HOOK] = -1;
	...
	if ( level.intermissiontime ) {
		...
		for ( i = WP_NUM_WEAPONS - 1 ; i > 0 ; i-- ) {
			if ( client->ps.stats[STAT_WEAPONS] & ( 1 << i ) && i !=WP_GRAPPLING_HOOK && i !=WP_PROX_LAUNCHER ) {
				client->ps.weapon = i;
				break;
			}
		}
