...
static void PlayerIntroSound( const char *modelAndSkin ) {
	...
	Q_strncpyz( model, modelAndSkin, sizeof(model) );
        ...
        skin = va( "play sound/player/announce/%s.wav\n", modelnospaces );
	trap_SendConsoleCommand( EXEC_APPEND, skin );
}
...
void G_CheckBotSpawn( void ) {
	...
	for( n = 0; n < BOT_SPAWN_QUEUE_DEPTH; n++ ) {
		...
		//if( g_gametype.integer == GT_SINGLE_PLAYER ) {
			...
			PlayerIntroSound( Info_ValueForKey (userinfo, "name") );
		//}
	}
}
...
static void G_AddBot( const char *name, float skill, const char *team, int delay, char *altname) {
	...
	if ( Q_strequal( name, "random" ) ) {
		...
		botinfo = G_GetBotInfoByNumber( botinfoNum );
                if ( botinfo ) {
                        botname = Info_ValueForKey( botinfo, "special" );
                        if (botname[0]) {
...
void Svcmd_AddBot_f( void ) {
	...
	if ( !string[0] ) {
		skill = 4;
                ...
                skill = atof( string );
                if (skill == 0) {
                  skill = Com_Clamp( 1, 5, g_spSkill.integer );
                } else {
