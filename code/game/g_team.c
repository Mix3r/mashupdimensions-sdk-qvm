...
void Team_Dom_SpawnPoints( void )
{
	...
	it = NULL;
	it = BG_FindItem ("team_dom_pointWhite");
	if(it == NULL) {
...
void Team_Dom_TakePoint( gentity_t *point, int team, int clientnumber )
{
	...
	if(team == TEAM_RED) {
		it = BG_FindItem ("team_dom_pointRed");
		PrintMsg( NULL, "Red took \'%s\'\n",level.domination_points_names[i]);
	}
	else if(team == TEAM_BLUE) {
		it = BG_FindItem ("team_dom_pointBlue");
...
void Team_DD_makeA2team( gentity_t *target, int team )
{
	...
	if(team == TEAM_NONE || !target) {
		...
		it = BG_FindItem ("team_DD_pointAred");
	}
	else if(team == TEAM_BLUE) {
		it = BG_FindItem ("team_DD_pointAblue");
	}
	else if(team == TEAM_FREE) {
		it = BG_FindItem ("team_DD_pointAwhite");
	}
	if (!it || it == NULL) {
		PrintMsg( NULL, "No item\n");
		return;
...
static void Team_DD_makeB2team( gentity_t *target, int team )
{
	...
	if(team == TEAM_RED) {
		it = BG_FindItem ("team_DD_pointBred");
	}
	if(team == TEAM_BLUE) {
		it = BG_FindItem ("team_DD_pointBblue");
	}
	if(team == TEAM_FREE) {
		it = BG_FindItem ("team_DD_pointBwhite");
	}
	if (!it || it == NULL) {
		PrintMsg( NULL, "No item\n");
		return;
