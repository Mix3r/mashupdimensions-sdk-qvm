...
static void ClientTimerActions( gentity_t *ent, int msec ) {
	...
	while ( client->timeResidual <= level.time ) {
                client->timeResidual = level.time - client->timeResidual;
                if (client->timeResidual > 1000) {
                        client->timeResidual = 0;
                }
		client->timeResidual = level.time+(1000-client->timeResidual);
		...
		if( maxHealth ) {
			...
			if ( G_IsARoundBasedGametype(g_gametype.integer) && level.humansEliminated ) {
                                if (g_bot_selfdestruct.integer) {
				        ent->damage=g_bot_selfdestruct.integer; // Mix3r_Durachor: it was set to 50
...
//unlagged - true ping
	...
	// go through as an attack unless it actually hits something
        ...
        if (( client->ps.weapon == WP_GAUNTLET) && !( ucmd->buttons & BUTTON_TALK )) {
           if (client->ps.ammo[ client->ps.weapon ] == -2) {    // mix3r: hack to translate attack from bg_pmove.c
                client->ps.ammo[ client->ps.weapon ] = -1;
