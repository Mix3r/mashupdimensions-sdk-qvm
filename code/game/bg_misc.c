...
gitem_t bg_itemlist[] ={
	 ...
	 */
	{
		...
		{ "models/powerups/instant/airhog_chassis.md3",
...
gitem_t *BG_FindItem(const char *pickupName) {
	...
	for (it = bg_itemlist + 1; it->classname; it++) {
		if (Q_strequal(it->classname, pickupName))
			return it;
	}
...
qboolean BG_CanItemBeGrabbed(int gametype, const entityState_t *ent, const playerState_t *ps) {
	...
	switch (item->giType) {
		...
		case IT_AMMO:
                        upperBound = atoi(item->precaches);
                        if (upperBound == 0) {
                                upperBound = 200;
                        }
			if (ps->ammo[ item->giTag ] >= upperBound) {
