...
qboolean CheckGauntletAttack( gentity_t *ent )
{
	...
	if ( !traceEnt->takedamage) {
        ...
        damage = EV_MISSILE_HIT;

        // send impact
	if ( traceEnt->client ) {
                // Mix3r_Durachok: check melee block feature to reduce damage to 10%
                if (traceEnt->client->pers.nMeleeBlockTimestamp >= level.time) {
                        if (traceEnt->client->pers.nMeleeBlockPawn == ent->client->ps.clientNum) {
                                damage = EV_MISSILE_MISS;
                        }
                }
		tent = G_TempEntity( tr.endpos, damage );
	...
	if (g_instantgib.integer) {
		damage = (damage * s_quadFactor) + 94; //High damage in instant gib (normally enough to gib)
        } else {
	        damage *= s_quadFactor;
        }
...
qboolean ShotgunPellet( vec3_t start, vec3_t end, gentity_t *ent )
{
	...
	for (i = 0; i < 10; i++) {
		...
		if ( traceEnt->takedamage) {
                        if ( oldTarg[countTarg-1] != traceEnt && countTarg < 10 ) {
				oldTarg[countTarg] = traceEnt;
				oldTarg[countTarg]->sumImpact_dmg = 0;
				countTarg++;
			}
...
//unlagged - backward reconciliation #2
	...
	for ( i = 0 ; i < DEFAULT_SHOTGUN_COUNT ; i++ ) {
		...
		if ( oldTarg[i]->sumImpact_dmg && oldTarg[i] != oldAttacker ) {
			ScorePlum( oldAttacker, oldTarg[i]->r.currentOrigin, oldTarg[i]->sumImpact_dmg, 1 ); /// enhance this later
		}
	}
	if( hitClient )
