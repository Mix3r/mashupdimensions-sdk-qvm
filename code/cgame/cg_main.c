...
qboolean regsnd_flag; // Mix3r_Durachok: flag to register sound. some sounds use qfalse instead of commonly used qtrue
...
"Point A (White)",
"Point B (White)",
"Neutral domination point",
"Red domination point",
"Blue domination point",
"Tied for ",
"%s place with %i",
"Teams are tied for %i",
"Red leads - %i vs %i",
"Blue leads - %i vs %i",
"Spectator. ^2Fire^7 - follow.",
"Waiting for your turn.",
"Press ^2ESC^7 and pick a side by using [TEAM] menu.",
"^2Use^7 - stop following",
"Following %s",
"VOTE(%i):%s Yes (F1):%i No (F2):%i",
"TEAM VOTE(%i):%s Yes:%i No:%i",
"Waiting for party",
"%s vs %s",
"Begins in: %i",
"Click FIRE to respawn",
"Respawn in: %2.2f",
" connecting %s",
" Spec %3i %4i %s",
"Skull Trumpet",
...
"Xen",
"MISSION",
" SCORE |  PING |  TIME |NAME ", // Mix3r_Durachok: 88 is special lang prefix
"Major",
"Sarge",
"Bachelor",
"Drondik",
...
"Делите ",
"%s место c %i",
"Счёт команд равный - %i",
"Красные ведут - %i к %i",
"Синие ведут - %i к %i",
"Зритель. ^2Огонь^7 - следить.",
"Ожидание своей очереди.",
"Нажмите ^2ESC^7, выберите команду в меню [Участие].",
"^2Применить^7 - перестать следить",
"Следим за %s",
"ГОЛОСУЕМ(%i):%s Да (F1):%i Нет (F2):%i",
"КОМАНДНЫЕ ВЫБОРЫ(%i):%s Да:%i Нет:%i",
"Ожидание участников",
"%s против %s",
"До начала: %i",
"Нажмите ОГОНЬ для возрождения",
"Возрождение через: %2.2f",
" заходит    %s",
" Зрит %3i %4i %s",
"Мистер Дудец",
"Паукан",
...
"Ксен",
"ЗАДАНИЕ",
"  СЧЁТ |  ПИНГ | ВРЕМЯ |ИМЯ  ", // Mix3r_Durachok: 88 is special lang prefix
"Корнет",
"Поручик",
"Бакc",
"Язь",
...
static cvarTable_t cvarTable[] = {// bk001129
	...
	{ &cg_zoomFov, "cg_zoomfov", "45", CVAR_ARCHIVE},
	...
	{ &cg_thirdPersonRange, "cg_thirdPersonRange", "75", CVAR_CHEAT},
	...
	//unlagged - client options
	{ &cg_trueLightning, "cg_trueLightning", "0.8", CVAR_ARCHIVE},
...
void CG_RegisterCvars(void) {
	...
	cgs.localServer = atoi(var);
        ...
        // Mix3r_Durachok: set pmove behavior depending of com_maxfps value:
        trap_Cvar_VariableStringBuffer("com_maxfps", var, sizeof (var));
        if (atoi(var) == 125) {
...
static void CG_RegisterItemSounds(int itemNum) {
	...
	if (item->pickup_sound) {
                regsnd_flag = qfalse;
		CG_CacheLocalizedSound(item->pickup_sound);
	}
...
static void CG_RegisterSounds(void) {
...
#endif
        regsnd_flag = qtrue;
	cgs.media.oneMinuteSound = CG_CacheLocalizedSound("sound/feedback/1_minute.wav");
	cgs.media.fiveMinuteSound = CG_CacheLocalizedSound("sound/feedback/5_minute.wav");
	cgs.media.suddenDeathSound = CG_CacheLocalizedSound("sound/feedback/sudden_death.wav");
	cgs.media.oneFragSound = CG_CacheLocalizedSound("sound/feedback/1_frag.wav");
	cgs.media.twoFragSound = CG_CacheLocalizedSound("sound/feedback/2_frags.wav");
	cgs.media.threeFragSound = CG_CacheLocalizedSound("sound/feedback/3_frags.wav");
	cgs.media.count3Sound = CG_CacheLocalizedSound("sound/feedback/three.wav");
	cgs.media.count2Sound = CG_CacheLocalizedSound("sound/feedback/two.wav");
	cgs.media.count1Sound = CG_CacheLocalizedSound("sound/feedback/one.wav");
	cgs.media.countFightSound = CG_CacheLocalizedSound("sound/feedback/fight.wav");
	cgs.media.countPrepareSound = CG_CacheLocalizedSound("sound/feedback/prepare.wav");
#ifdef MISSIONPACK
	...
	if (CG_IsATeamGametype(cgs.gametype) || cg_buildScript.integer) {
		...
		cgs.media.redLeadsSound = CG_CacheLocalizedSound("sound/feedback/redleads.wav");
		cgs.media.blueLeadsSound = CG_CacheLocalizedSound("sound/feedback/blueleads.wav");
		cgs.media.teamsTiedSound = CG_CacheLocalizedSound("sound/feedback/teamstied.wav");
		cgs.media.hitTeamSound = trap_S_RegisterSound("sound/feedback/hit_teammate.wav", qtrue);

		cgs.media.redScoredSound = CG_CacheLocalizedSound("sound/teamplay/voc_red_scores.wav");
		cgs.media.blueScoredSound = CG_CacheLocalizedSound("sound/teamplay/voc_blue_scores.wav");
		...
		if ((CG_UsesTeamFlags(cgs.gametype) && !CG_UsesTheWhiteFlag(cgs.gametype)) || cg_buildScript.integer) {
			cgs.media.redFlagReturnedSound = CG_CacheLocalizedSound("sound/teamplay/voc_red_returned.wav");
			cgs.media.blueFlagReturnedSound = CG_CacheLocalizedSound("sound/teamplay/voc_blue_returned.wav");
			cgs.media.enemyTookYourFlagSound = CG_CacheLocalizedSound("sound/teamplay/voc_enemy_flag.wav");
			cgs.media.yourTeamTookEnemyFlagSound = CG_CacheLocalizedSound("sound/teamplay/voc_team_flag.wav");
		}

		if (CG_UsesTheWhiteFlag(cgs.gametype) || cg_buildScript.integer) {
			// FIXME: get a replacement for this sound ?
			cgs.media.neutralFlagReturnedSound = trap_S_RegisterSound("sound/teamplay/flagreturn_opponent.wav", qtrue);
			cgs.media.yourTeamTookTheFlagSound = CG_CacheLocalizedSound("sound/teamplay/voc_team_1flag.wav");
			cgs.media.enemyTookTheFlagSound = CG_CacheLocalizedSound("sound/teamplay/voc_enemy_1flag.wav");
		}

		if (cgs.gametype == GT_OBELISK || cg_buildScript.integer) {
			cgs.media.yourBaseIsUnderAttackSound = CG_CacheLocalizedSound("sound/teamplay/voc_base_attack.wav");
			// loadingscreen
#ifdef SCRIPTHUD
	...
	if (CG_UsesTeamFlags(cgs.gametype) || CG_UsesTheWhiteFlag(cgs.gametype) || cg_buildScript.integer) {
		cgs.media.youHaveFlagSound = CG_CacheLocalizedSound("sound/teamplay/voc_you_flag.wav");
		cgs.media.holyShitSound = CG_CacheLocalizedSound("sound/feedback/voc_holyshit.wav");
	}
	...
	cgs.media.invulnerabilityJuicedSound = trap_S_RegisterSound("sound/items/invul_juiced.wav", qfalse);
        ...
        regsnd_flag = qfalse;
        cgs.media.ammoregenSound = CG_CacheLocalizedSound("sound/items/cl_ammoregen.wav");
	cgs.media.doublerSound = CG_CacheLocalizedSound("sound/items/cl_doubler.wav");
	cgs.media.guardSound = CG_CacheLocalizedSound("sound/items/cl_guard.wav");
	cgs.media.scoutSound = CG_CacheLocalizedSound("sound/items/cl_scout.wav");
        regsnd_flag = qtrue;

	cgs.media.obeliskHitSound1 = trap_S_RegisterSound("sound/items/obelisk_glass_01.wav", qfalse);
	cgs.media.obeliskHitSound2 = trap_S_RegisterSound("sound/items/obelisk_glass_02.wav", qfalse);
	cgs.media.obeliskHitSound3 = trap_S_RegisterSound("sound/items/obelisk_glass_03.wav", qfalse);
	cgs.media.obeliskRespawnSound = trap_S_RegisterSound("sound/items/obelisk_respawn.wav", qfalse);
        //cgs.media.objHitSound1 = trap_S_RegisterSound("sound/feedback/hit_obj1.wav", qfalse);
	//cgs.media.objHitSound2 = trap_S_RegisterSound("sound/feedback/hit_obj2.wav", qfalse);
	//cgs.media.objHitSound3 = trap_S_RegisterSound("sound/feedback/hit_obj3.wav", qfalse);
	...
	cgs.media.noAmmoSound = trap_S_RegisterSound("sound/weapons/noammo.wav", qfalse);
        cgs.media.MeleeHitSound = trap_S_RegisterSound("sound/weapons/melee/fstatck.wav", qfalse);
        cgs.media.MeleeBlocked1 = trap_S_RegisterSound("sound/weapons/melee/fstblock1.wav", qfalse);
        cgs.media.MeleeBlocked2 = trap_S_RegisterSound("sound/weapons/melee/fstblock2.wav", qfalse);

	cgs.media.talkSound = trap_S_RegisterSound("sound/player/talk.wav", qfalse);
	cgs.media.landSound = trap_S_RegisterSound("sound/player/land1.wav", qfalse);
        if (cgs.media.landSound) {
        } else {
                cgs.media.landSound = trap_S_RegisterSound("sound/player/land_a.wav", qfalse);
                cgs.media.landbSound = trap_S_RegisterSound("sound/player/land_b.wav", qfalse);
                cgs.media.jumpbSound = trap_S_RegisterSound("sound/player/jump_b.wav", qfalse);
        }
	...
	cgs.media.impressiveSound = CG_CacheLocalizedSound("sound/feedback/impressive.wav");
	cgs.media.excellentSound = CG_CacheLocalizedSound("sound/feedback/excellent.wav");
	cgs.media.deniedSound = CG_CacheLocalizedSound("sound/feedback/denied.wav");
	cgs.media.humiliationSound = CG_CacheLocalizedSound("sound/feedback/humiliation.wav");
	cgs.media.assistSound = CG_CacheLocalizedSound("sound/feedback/assist.wav");
	cgs.media.defendSound = CG_CacheLocalizedSound("sound/feedback/defense.wav");
#ifdef MISSIONPACK
	...
	cgs.media.takenLeadSound = CG_CacheLocalizedSound("sound/feedback/takenlead.wav");
	cgs.media.tiedLeadSound = CG_CacheLocalizedSound("sound/feedback/tiedlead.wav");
	cgs.media.lostLeadSound = CG_CacheLocalizedSound("sound/feedback/lostlead.wav");
	...
	for (i = 1; i < MAX_SOUNDS; i++) {
		...
		if (soundName[0] == '*') {
			continue; // custom sound
                ...
                for (n = 1; n < 3; n++) {
                        cgs.gameSounds[i] = trap_S_RegisterSound(va("%s%s",COM_Localize(n),soundName), qfalse);
                        if (cgs.gameSounds[i]) {
                                break;
                        }
                }
    ...
    cgs.media.UwFireSound = trap_S_RegisterSound("sound/weapons/uwfire1.wav", qfalse);
	...
	cgs.media.sfx_chghitflesh = trap_S_RegisterSound("sound/weapons/mma_b_hit.wav", qfalse);
...
static void CG_RegisterGraphics(void) {
	...
	if (CG_UsesTeamFlags(cgs.gametype) || cgs.gametype == GT_HARVESTER || cg_buildScript.integer) {
		...
		cgs.media.blueCubeModel = trap_R_RegisterModel("models/powerups/orb/b_orb.md3");
                if (trap_R_RegisterShaderNoMip("levelshots/mpteam1_small.tga")) {
		        cgs.media.redCubeIcon = trap_R_RegisterShader("icons/skull_red");
		        cgs.media.blueCubeIcon = trap_R_RegisterShader("icons/skull_blue");
                } else {
                        cgs.media.redCubeIcon = trap_R_RegisterShaderNoMip("icons/iconh_rorb.tga");
                }       cgs.media.blueCubeIcon = trap_R_RegisterShaderNoMip("icons/iconh_borb.tga");
	...
	if (cgs.gametype == GT_OBELISK || cg_buildScript.integer) {
		...
		cgs.media.overloadTargetModel = trap_R_RegisterModel("models/powerups/overload_targetmma.md3");
		...
		cgs.media.overloadEnergyModel = trap_R_RegisterModel("models/powerups/overload_energy.md3");
                cgs.media.blueCubeIcon = trap_R_RegisterShaderNoMip("gfx/misc/moonshinersbar.tga");
	}
	...
	cgs.media.teamStatusBar = trap_R_RegisterShaderNoMip("gfx/2d/colorbar.tga");

	//cgs.media.armorModel = trap_R_RegisterModel("models/powerups/armor/armor_yel.md3");
	cgs.media.armorIcon = trap_R_RegisterShaderNoMip("icons/iconr_yellow");
        cgs.media.armorIconRed = trap_R_RegisterShaderNoMip("icons/iconr_red");
        cgs.media.armorIconShard = trap_R_RegisterShaderNoMip("icons/iconr_shard");
...
void CG_Init(int serverMessageNum, int serverCommandSequence, int clientNum) {
	...
	cg.clientNum = clientNum;
        ...
        if (trap_R_RegisterShaderNoMip("levelshots/Q3DM0.jpg")) {
                for ( cg.weaponSelect = 0; cg.weaponSelect < MAX_WEAPONS; cg.weaponSelect++ ) {
                        cg.headEndPitch[cg.weaponSelect] = 2.0;
                }
                // Mix3r_Durachok: raise q3 LG model for 1st person view a bit more:
                cg.headEndPitch[WP_LIGHTNING] = 3.2;
        } else if (trap_R_RegisterShaderNoMip("levelshots/mpteam1_small.tga")) {
                for ( cg.weaponSelect = 0; cg.weaponSelect < MAX_WEAPONS; cg.weaponSelect++ ) {
                        cg.headEndPitch[cg.weaponSelect] = 0.0;
                }
                // Mix3r_Durachok: raise native teamarena weapon models:
                cg.headEndPitch[WP_NAILGUN] = cg.headEndPitch[WP_PROX_LAUNCHER] = cg.headEndPitch[WP_CHAINGUN] = 2.0;
        } else {
                for ( cg.weaponSelect = 0; cg.weaponSelect < MAX_WEAPONS; cg.weaponSelect++ ) {
                        cg.headEndPitch[cg.weaponSelect] = 0.0;
                }
	...
	cgs.media.charsetShader = trap_R_RegisterShader("gfx/2d/bigchar2");
...

	{
		...
		wideAdjustX = 0.5 * (newresx - (newresy * (640.0 / 480.0)));
