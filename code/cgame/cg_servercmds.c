...
void CG_SetConfigValues( void ) {
	...
	if (CG_UsesTeamFlags(cgs.gametype) || cgs.gametype == GT_DOUBLE_D) {
                cgs.redflag = atoi(CG_ConfigString( CS_FLAGSTATUS ));
                if (cgs.redflag > 9) {
                        cgs.blueflag = (int)cgs.redflag%10;
                        cgs.redflag = (int)cgs.redflag/10;
                } else {
                        cgs.blueflag = 0;
                }
	}
...
static void CG_ConfigStringModified( void ) {
	...
	if ( num == CS_MUSIC ) {
		...
		cgs.scores2 = atoi( str );
                ...
                // Mix3r_Durachok: hack (to prevent blue score is -9999
                // after change from ffa to teamplay on next map)
                if (cgs.scores2 == -9999) {
                        cgs.scores2 = 0;
                }
                if (cgs.gametype == GT_TEAM) {
		...
		cg.intermissionStarted = atoi( str );
                if (cg.intermissionStarted > 0) {
                        cg.intermissionStarted = cg.time - cgs.levelStartTime;
                }
		...
		if ( str[0] != '*' ) {	// player specific sounds don't register here
                        for (n = 1; n < 3; n++) {
                                cgs.gameSounds[ num-CS_SOUNDS] = trap_S_RegisterSound( va("%s%s",COM_Localize(n),str), qfalse );
                                if (cgs.gameSounds[ num-CS_SOUNDS]) {
                                        break;
                                }
                        }
		}
		...
		if( CG_UsesTheWhiteFlag(cgs.gametype) ) {
			cgs.redflag = num;
                        // CG_Printf( "STATUS: %s\n", str );
		} else if (CG_UsesTeamFlags(cgs.gametype) || cgs.gametype == GT_DOUBLE_D) {
                        cgs.redflag = (int)num/10;
			cgs.blueflag = (int)num%10;
                        // CG_Printf( "STATUS: %s\n", str );
                }
