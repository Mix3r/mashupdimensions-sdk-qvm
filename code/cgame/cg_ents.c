...
static void CG_Item( centity_t *cent ) {
	...
	if ( cg_simpleItems.integer && item->giType != IT_TEAM ) {
		...
		return;
                ...
                if ( item->giType == IT_HEALTH ) {
                        cent->lerpAngles[YAW] = cg.autoAnglesFast[YAW];
                } else if ( item->giType == IT_WEAPON ) {
                        cent->lerpAngles[YAW] = cg.autoAngles[YAW];
                        frac = 90.0;
                } else {
                        cent->lerpAngles[YAW] = cg.autoAngles[YAW];
                }
                if (cg_shadows.integer == 1) {
                        //CG_Printf("sta: [%.4f] 1: [%.4f] 2: [%.4f]\n", cent->lerpOrigin[0], cent->lerpOrigin[1], cent->lerpOrigin[2] );
                        cent->lerpOrigin[2]-=15;
                        CG_ImpactMark(cgs.media.shadowMarkShader, cent->lerpOrigin, axisDefault[2],cent->lerpAngles[YAW]+frac, 1, 1, 1, 0.33, qfalse, 18, qtrue);
                        cent->lerpOrigin[2]+=15;
                }
        }

	// items bob up and down continuously
	frac = 0.005 + cent->currentState.number * 0.00001;
	cent->lerpOrigin[2] += 4 + cos( ( cg.time + 1000 ) *  frac ) * 4;
	...
	if ( item->giType == IT_HEALTH ) {
        ...
        //ent.origin[2]=cent->lerpOrigin[2]-120;
        //ent.lightingOrigin[0]=ent.lightingOrigin[1]=-15.0;
        //ent.oldorigin[0]=ent.oldorigin[1]=15.0;
        //ent.lightingOrigin[2]=0.0;
        //ent.oldorigin[2]=2.0;

        //trap_CM_BoxTrace(&trace, cent->lerpOrigin, ent.origin, ent.lightingOrigin, ent.oldorigin, 0, MASK_PLAYERSOLID);
        //if (trace.fraction == 1.0 || trace.startsolid || trace.allsolid) {
	//} else {
        //        ent.oldorigin[0] = 0.0;
        //        ent.oldorigin[1] = 0.0;
        //        ent.oldorigin[2] = 1.0;
        //        CG_ImpactMark(cgs.media.shadowMarkShader, trace.endpos, ent.oldorigin,cent->lerpAngles[YAW], 1, 1, 1, 0.5, qfalse, 24, qtrue);
        //        //CG_ImpactMark(cgs.media.shadowMarkShader, trace.endpos, axisDefault[3],cent->lerpAngles[YAW], 1, 1, 1, 0.5, qfalse, 24, qtrue);
        //        CG_Printf("cosgles end: [%.4f] 1: [%.4f] 2: [%.4f]\n", trace.endpos[0], trace.endpos[1], trace.endpos[2] );
        //}
        //////////////////////////////
