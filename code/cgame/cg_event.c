...
const char *CG_PlaceString(int rank) {
	...
	if (rank == 1) {
		s = S_COLOR_GREEN "1" S_COLOR_WHITE; // draw in green
	} else if (rank == 2) {
		s = S_COLOR_RED "2" S_COLOR_WHITE; // draw in red
	} else if (rank == 3) {
		s = S_COLOR_YELLOW "3" S_COLOR_WHITE; // draw in yellow
	} else {
		s = va("%i", rank);
	}
...
static void CG_Obituary(entityState_t *ent) {
	...
	if (!targetInfo) {
        ...
        if (ci->botSkill > 0 && ci->handicap > 0) {
                Q_strncpyz(targetName, COM_Localize(ci->handicap), sizeof (targetName) - 2);
        } else {
	...
	//if (attacker == cg.snap->ps.clientNum) {
		...
		//	s = va("Bы cpaзили %s. %s мecтo c %i", targetName,
		//	CG_PlaceString(cg.snap->ps.persistant[PERS_RANK] + 1),cg.snap->ps.persistant[PERS_SCORE]);
		//} else {
		//	if (ent->generic1)
		//		s = va("Вы сразили ^1СОЮЗНИКА^7 %s", targetName);
	//		else
		//		s = va("Вы сразили %s", targetName);
		//}
#ifdef MISSIONPACK
	//	if (!(cg_singlePlayerActive.integer && cg_cameraOrbit.integer)) {
	//		CG_CenterPrint(s, SCREEN_HEIGHT * 0.30, (int) (10 * cg_fragmsgsize.value));
	//	}
#else
		//CG_CenterPrint(s, 56 - 10, 8);
#endif
	...
	if (!attackerInfo) {
		...
		strcpy(attackerName, "noname");
                ...
                if (ci->botSkill > 0 && ci->handicap > 0) {
                        Q_strncpyz(attackerName, COM_Localize(ci->handicap), sizeof (attackerName) - 2);
                } else {
			...
			if (cg_obituaryOutput.integer >= 2) {
				...
				Q_strncpyz(lastFrag->attackerName, attackerName,
						sizeof (lastFrag->attackerName));
                                lastFrag->widths_at[0] = CG_DrawStrlen(lastFrag->attackerName);

                                if (attacker == cg.snap->ps.clientNum) {
                                        lastFrag->widths_at[1] = CG_DrawStrlen(lastFrag->targetName);
                                        cg.headYaw = cg.time;
                                } else {
                                        lastFrag->widths_at[1] = 0;
                                }
...
static void CG_UseItem(centity_t *cent) {
	...
	if (es->number == cg.snap->ps.clientNum) {
		if (!itemNum) {
			CG_CenterPrint(COM_Localize(94), SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH);
		} else {
			item = BG_FindItemForHoldable(itemNum);
			CG_CenterPrint(va(COM_Localize(95), item->pickup_name), SCREEN_HEIGHT * 0.30, BIGCHAR_WIDTH);
		}
	}
	...
	switch (event) {
			...
			DEBUGNAME("EV_GLOBAL_ITEM_PICKUP");
		{
			...
			if (item->pickup_sound) {
                                if (item->giType == IT_POWERUP) {
                                        regsnd_flag = qfalse;
				        trap_S_StartSound(NULL, cg.snap->ps.clientNum, CHAN_AUTO, CG_CacheLocalizedSound(item->pickup_sound));
                                } else {
		...
		case EV_SCOREPLUM:
			//DEBUGNAME("EV_SCOREPLUM");
                        switch(cent->currentState.weapon) {
                                case 10:if (!cgs.media.bfgExplosionShader) {
                                               cgs.media.bfgExplosionShader = trap_R_RegisterShader( "bfgExplosion" );
                                       }
                                       CG_MissileHitWall(WP_BFG, 0, cent->lerpOrigin, cg.refdef.viewaxis[1], IMPACTSOUND_DEFAULT);
                                       break;
                                case 11:CG_KamikazeEffect(cent->lerpOrigin);
                                       break;
                               default:CG_ScorePlum(cent->currentState.otherEntityNum, cent->lerpOrigin, cent->currentState.time, cent->currentState.weapon);
                                       break;
                        }
			break;
		...
		case EV_MISSILE_HIT:
			...
			ByteToDir(es->eventParm, dir);
                        ...
                        if (es->weapon == WP_GAUNTLET) {
                           trap_S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.MeleeHitSound);
                        }
		...
		case EV_MISSILE_MISS:
			...
			ByteToDir(es->eventParm, dir);
                        ...
                        if (es->weapon == WP_GAUNTLET) {
                           if (cg.time%2 == 0) {
                                   trap_S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.MeleeBlocked1);
                           } else {
                                   trap_S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.MeleeBlocked2);
                           }
                        }
		...
		case EV_RAILTRAIL:
			...
			if (es->clientNum == cg.predictedPlayerState.clientNum &&
				...
				CG_RailTrail(ci, cgs.clientinfo[ es->clientNum ].pGunPoint, es->pos.trBase);
		...
		case EV_GLOBAL_SOUND: // play from the player's head so it never diminishes
			...
			if (cgs.gameSounds[ es->eventParm ]) {
				trap_S_StartLocalSound(cgs.gameSounds[ es->eventParm ], CHAN_AUTO);
                                //Com_Printf("Distant weird play snd!!!!!!!!!!!!!!!!!!!\n");
			} else {
				s = CG_ConfigString(CS_SOUNDS + es->eventParm);
				trap_S_StartSound(NULL, cg.snap->ps.clientNum, CHAN_AUTO, CG_CustomSound(es->number, s));
                                //trap_SendConsoleCommand( va( "play %s\n", s ) );
                                //Com_Printf("Test console play snd!!!!!!!!!!!!!!!!!!!\n");
			}
			break;
