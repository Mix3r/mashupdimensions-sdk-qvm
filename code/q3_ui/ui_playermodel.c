...
#define MODEL_FRAMEL		"menu/" MENU_ART_DIR "/frame1_r_mma3"
#define MODEL_FRAMER		"menu/" MENU_ART_DIR "/frame1_r_mma2"
...
#define MODEL_HEAD0		"menu/" MENU_ART_DIR "/head_0"
#define MODEL_HEAD1		"menu/" MENU_ART_DIR "/head_1"
...
#define ID_BACK				102
#define ID_RECAPITATE			99
#define ID_RECAPITATE2			98
...
typedef struct
{
	...
	int		numpages;
	char		modelskin[64];
        char		headmodelskin[64];
	int		selectedmodel;

        menubitmap_s	headmdl;
...
static void PlayerModel_DrawBGart( void *self ) {
       if (s_playermodel.bg_art.generic.x == 1) {
                trap_R_DrawStretchPic( 0.0, 0.0, uis.glconfig.vidWidth, uis.glconfig.vidHeight, 0, 0, 1, 1, s_playermodel.framer );
       } else {
                trap_R_DrawStretchPic( 0.0, 0.0, uis.glconfig.vidWidth, uis.glconfig.vidHeight, 0, 0, 1, 1, s_playermodel.framel );
       }
}
...
static void PlayerModel_UpdateModel( void )
{
	...
	UI_PlayerInfo_SetModel( &s_playermodel.playerinfo, s_playermodel.modelskin, s_playermodel.headmodelskin );
...
static void PlayerModel_SaveChanges( void )
{
	...
	trap_Cvar_Set( "team_headmodel", s_playermodel.headmodelskin );
}
	...
	switch (((menucommon_s*)ptr)->id)
	{
		...
		case ID_BACK:
			...
			break;
                case ID_RECAPITATE:
                        if (s_playermodel.bg_art.generic.x == 1) {
                                s_playermodel.bg_art.generic.x = 2;
                                s_playermodel.banner.string        = "ГОЛОВА ИГРОКА";
                                s_playermodel.headmdl.shader = trap_R_RegisterShaderNoMip( MODEL_HEAD0 );
                                s_playermodel.headmdl.focusshader = trap_R_RegisterShaderNoMip( MODEL_HEAD1 );

                        } else {
                                s_playermodel.bg_art.generic.x = 1;
                                s_playermodel.banner.string        = "МОДЕЛЬ ИГРОКА";
                                s_playermodel.headmdl.shader = trap_R_RegisterShaderNoMip( ART_MODEL0 );
                                s_playermodel.headmdl.focusshader = trap_R_RegisterShaderNoMip( ART_MODEL1 );
                        }
			break;
	}
}
	...
	if (pdest)
	{
		// track the whole model/skin name
		        ...
		        strcat(s_playermodel.modelskin,pdest + 5);
                        ...
                        Q_strncpyz(s_playermodel.headmodelskin,buffptr,pdest-buffptr+1);
		        strcat(s_playermodel.headmodelskin,pdest + 5);
                }
...
static void PlayerModel_SetMenuItems( void )
{
	...
	trap_Cvar_VariableStringBuffer( "model", s_playermodel.modelskin, 64 );
        ...
        // headmodel
        trap_Cvar_VariableStringBuffer( "headmodel", s_playermodel.headmodelskin, 64 );
	
	// use default skin if none is set
	if (!strchr(s_playermodel.modelskin, '/')) {
		Q_strcat(s_playermodel.modelskin, 64, "/default");
	}
        if (!strchr(s_playermodel.headmodelskin, '/')) {
		Q_strcat(s_playermodel.headmodelskin, 64, "/default");
	}
...
static void PlayerModel_MenuInit( void )
{
	...
	s_playermodel.menu.fullscreen = qtrue;
        ...
        s_playermodel.bg_art.generic.type	= MTYPE_BITMAP;
	s_playermodel.bg_art.generic.flags	= QMF_INACTIVE;
	s_playermodel.bg_art.generic.ownerdraw	= PlayerModel_DrawBGart;
	s_playermodel.bg_art.generic.x		= 1;
	s_playermodel.bg_art.generic.y		= 1;
	s_playermodel.bg_art.width		= 2;
	s_playermodel.bg_art.height		= 2;
	...
	s_playermodel.back.focuspic         = MODEL_BACK1;
        ...
        s_playermodel.headmdl.generic.type     = MTYPE_BITMAP;
	s_playermodel.headmdl.generic.name     = ART_MODEL0;
	s_playermodel.headmdl.generic.flags    = QMF_LEFT_JUSTIFY|QMF_PULSEIFFOCUS;
	s_playermodel.headmdl.generic.callback = PlayerModel_MenuEvent;
	s_playermodel.headmdl.generic.id	    = ID_RECAPITATE;
	s_playermodel.headmdl.generic.x		= 640-128;
	s_playermodel.headmdl.generic.y		= 480-64;
	s_playermodel.headmdl.width		= 128;
	s_playermodel.headmdl.height		= 64;
	s_playermodel.headmdl.focuspic         = ART_MODEL1;

        Menu_AddItem( &s_playermodel.menu,	&s_playermodel.bg_art );
	...
	Menu_AddItem( &s_playermodel.menu,	&s_playermodel.back );
        Menu_AddItem( &s_playermodel.menu,	&s_playermodel.headmdl );
...
void PlayerModel_Cache( void )
{
	...
	for( i = 0; i < s_playermodel.nummodels; i++ ) {
        ...
        s_playermodel.framel = trap_R_RegisterShaderNoMip( MODEL_FRAMEL );
        s_playermodel.framer = trap_R_RegisterShaderNoMip( MODEL_FRAMER );
}
