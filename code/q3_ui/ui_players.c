...
tryagain:
	...
	if ( weaponNum == WP_MACHINEGUN || weaponNum == WP_GAUNTLET || weaponNum == WP_BFG || weaponNum == WP_CHAINGUN ) {
...
void UI_DrawPlayer( float x, float y, float w, float h, playerInfo_t *pi, int time ) {
	...
	origin[0] = len / tan( DEG2RAD(refdef.fov_x) * 0.5 ) + 32; // + uis.cursory;
	...
	// add the legs
	//
        legs.shaderRGBA[0] = 0 * 255;
	legs.shaderRGBA[1] = 1 * 255;
	legs.shaderRGBA[2] = 0 * 255;
	...
	head.renderfx = renderfx;
        ...
        if (pi->animations[TORSO_ATTACK].firstFrame == pi->animations[LEGS_WALK].firstFrame) {
        } else {
	...
	if ( pi->currentWeapon != WP_NONE ) {
		...
		VectorCopy( origin, gun.lightingOrigin );
                if (pi->thirdpersonoffset[1] == 0.0) {
                        UI_PositionEntityOnTag( &gun, &legs, pi->legsModel, "tag_torso");
                        VectorMA(gun.origin, 12.0, legs.axis[0], gun.origin);

                        legs.skinNum = pi->legs.animationNumber & ~ANIM_TOGGLEBIT;
                                ...
                                default:
			        {
                        ...
                        VectorMA(gun.origin, head.axis[0][0], legs.axis[2], gun.origin);
                } else {
	...
	if ( pi->chat ) {
        ...
        legs.origin[2] -= 72;
        UI_PlayerFloatSprite( pi, legs.origin, trap_R_RegisterShaderNoMip( "sprites/char_art_shadow" ) );
	...
	origin[1] -= 100;
	origin[2] -= 100;
	trap_R_AddLightToScene( origin, -400, 1.0, 0.0, 0.0 );
...
static qboolean UI_ParseAnimationFile( const char *filename, playerInfo_t *pi ) {
	...
	pi->fixedtorso = qfalse;
        ...
        pi->thirdpersonoffset[0] = 0.0;
        pi->thirdpersonoffset[1] = 0.33;
        pi->thirdpersonoffset[2] = 15.0;
			...
			for ( i = 0 ; i < 3 ; i++ ) {
				...
				if ( !token[0] ) {
					break;
                                ...
                                pi->thirdpersonoffset[i] = atof(token);
				// do nothing
			}
			continue;
...
qboolean UI_RegisterClientModelname( playerInfo_t *pi, const char *modelSkinName, const char *headmodelSkinName ) {
	...
	if ( !UI_RegisterClientSkin( pi, modelName, skinName, headmodelName, headskinName ) ) {
		if ( !UI_RegisterClientSkin( pi, modelName, "default", modelName, "default" ) ) {
	...
	if ( !UI_ParseAnimationFile( filename, pi ) ) {
        ...
        //trap_Cvar_VariableStringBuffer( "headmodel", modelName, sizeof( modelName ) );
        //slash = strchr( modelName, '/' );
        //if ( slash ) {
        //        *slash = 0;
        //}
