...
static void setupMenuInfo_menu_draw( void ) {
        trap_R_DrawStretchPic( 0.0, 0.0, uis.glconfig.vidWidth, uis.glconfig.vidHeight, 0, 0, 1, 1, trap_R_RegisterShaderNoMip("menu/art_blueish/frame1_r_mma4") );
...
static void Setup_ResetDefaults_Draw( void ) {
	UI_DrawProportionalString( SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 0, COM_Localize(132), UI_CENTER|UI_SMALLFONT, color_yellow );
	UI_DrawProportionalString( SCREEN_WIDTH/2, 356 + PROP_HEIGHT * 1, COM_Localize(133), UI_CENTER|UI_SMALLFONT, color_yellow );
}
...
//		break;
	...
	case ID_DEFAULTS:
		UI_ConfirmMenu( COM_Localize(17), Setup_ResetDefaults_Draw, Setup_ResetDefaults_Action );
		break;
...
static void UI_SetupMenu_Init( void ) {
	...
	setupMenuInfo.setupplayer.string				= COM_Localize(127);
	...
	setupMenuInfo.setupcontrols.string				= COM_Localize(128);
	...
	setupMenuInfo.setupsystem.string				= COM_Localize(129);
	...
	setupMenuInfo.game.string						= COM_Localize(130);
...
/*	y += SETUP_MENU_VERTICAL_SPACING;
	...
	//if( !trap_Cvar_VariableValue( "cl_paused" ) ) {
		...
		setupMenuInfo.load.string						= "ЗАГРУЗИТЬ";
		...
		setupMenuInfo.save.string						= "СОХРАНИТЬ";
		...
		setupMenuInfo.defaults.string					= COM_Localize(131);
