...
static void ArenaServers_UpdateMenu( void ) {
	...
	if( g_arenaservers.numqueriedservers > 0 ) {
		...
		if( g_arenaservers.refreshservers && ( g_arenaservers.currentping <= g_arenaservers.numqueriedservers ) ) {
			...
			g_arenaservers.statusbar.string = COM_Localize(99);
			...
			if( (g_servertype >= UIAS_GLOBAL1 && g_servertype <= UIAS_GLOBAL5) || g_servertype == UIAS_ALL_GLOBAL ) {
				g_arenaservers.statusbar.string = COM_Localize(97);
			}
			else {
		...
		if( g_arenaservers.refreshservers ) {
			strcpy( g_arenaservers.status.string,COM_Localize(98) );
			g_arenaservers.statusbar.string = COM_Localize(99);
			...
			if( g_arenaservers.numqueriedservers < 0 ) {
				strcpy(g_arenaservers.status.string,COM_Localize(100) );
			}
			else {
				strcpy(g_arenaservers.status.string,COM_Localize(101) );
			}
...
static void ArenaServers_Insert( char* adrstr, char* info, int pingtime )
{
	...
	if ((pingtime >= ArenaServers_MaxPing()) && (g_servertype != UIAS_FAVORITES))
	{
        ...
        if (i < 100) {
                // Mix3r_Durachok: hack to filter non-Mashup-Dimensions based games
                // passed games have [gametype + 100] number, +100 added by server
                return;
        }
	...
	s = Info_ValueForKey( info, "game");
        //Com_Printf( "sname '%s'\n", s );
	i = atoi( Info_ValueForKey( info, "gametype") );
        i -= 100; // Mix3r_Durachok: to get correct gametype after using gametype+100 as passcode to recognize server as Mashup-Dimension-based
	if( i < 0 ) {
		i = 0;
	}
...
static void ArenaServers_MenuInit( void ) {
	...
	ArenaServers_Cache();
        ...
        gamenames[0] = COM_Localize(103);
        gamenames[3] = COM_Localize(104);
        gamenames[4] = COM_Localize(105);
        gamenames[5] = COM_Localize(106);
        gamenames[6] = COM_Localize(107);
        gamenames[7] = COM_Localize(108);
        gamenames[8] = COM_Localize(109);
        gamenames[9] = COM_Localize(110);
        gamenames[10] = COM_Localize(111);
        gamenames[11] = COM_Localize(112);
        gamenames[12] = COM_Localize(113);
        gamenames[13] = COM_Localize(114);
	...
	g_arenaservers.banner.string  		= COM_Localize(86);
	...
	g_arenaservers.master.generic.name			= COM_Localize(87);
	...
	g_arenaservers.master.itemnames				= master_items;
        ...
        servertype_items[0] = COM_Localize(102);
        servertype_items[1] = COM_Localize(32);
        servertype_items[2] = COM_Localize(33);
        servertype_items[3] = COM_Localize(34);
        servertype_items[4] = COM_Localize(35);
        servertype_items[5] = COM_Localize(36);
        servertype_items[6] = COM_Localize(37);
        servertype_items[7] = COM_Localize(38);
        servertype_items[8] = COM_Localize(39);
        servertype_items[9] = COM_Localize(40);
        servertype_items[10] = COM_Localize(41);
        servertype_items[11] = COM_Localize(42);
        servertype_items[12] = COM_Localize(43);
        servertype_items[13] = COM_Localize(44);

	y += SMALLCHAR_HEIGHT;
	g_arenaservers.gametype.generic.type		= MTYPE_SPINCONTROL;
	g_arenaservers.gametype.generic.name		= COM_Localize(31);
	...
	g_arenaservers.gametype.itemnames			= servertype_items;
        ...
        sortkey_items[0] = COM_Localize(53);
        sortkey_items[1] = COM_Localize(94);
        sortkey_items[2] = COM_Localize(95);
        sortkey_items[3] = COM_Localize(31);
        sortkey_items[4] = COM_Localize(93);
        sortkey_items[5] = COM_Localize(96);


	y += SMALLCHAR_HEIGHT;
	g_arenaservers.sortkey.generic.type			= MTYPE_SPINCONTROL;
	g_arenaservers.sortkey.generic.name			= COM_Localize(88);
	...
	g_arenaservers.showfull.generic.name		= COM_Localize(89);
	...
	g_arenaservers.showempty.generic.name		= COM_Localize(90);
	...
	g_arenaservers.onlyhumans.generic.name		= COM_Localize(91);
	...
	g_arenaservers.hideprivate.generic.name		= COM_Localize(92);
