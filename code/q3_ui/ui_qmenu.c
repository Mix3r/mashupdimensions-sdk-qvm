...
static void RadioButton_Draw( menuradiobutton_s *rb )
{
	...
	if ( !rb->curvalue )
	{
		...
		UI_DrawString( x + SMALLCHAR_WIDTH + 16, y, COM_Localize(154), style, color );
	}
	else
	{
		UI_DrawHandlePic( x + SMALLCHAR_WIDTH, y + 2, 16, 16, uis.rb_on );
		UI_DrawString( x + SMALLCHAR_WIDTH + 16, y, COM_Localize(155), style, text_color_highlight );
	}
}
...
void Menu_Cache( void )
{
	...
	//} else {
        ...
        if (ui_is_missionpack) {
                ...
                uis.menuBackShader = trap_R_RegisterShaderNoMip( va("textures/sfx/logo512%s",COM_Localize(1)) );
        }
