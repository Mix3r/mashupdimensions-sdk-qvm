...
static void VoteMenu_Fraglimit_Event( void* ptr, int event )
{
	...
	switch (((menucommon_s*)ptr)->id)
	{
        ...
        if (chosen_lim >= 0) {
                trap_Cmd_ExecuteText( EXEC_APPEND, va("callvote fraglimit %i",chosen_lim));
                UI_PopMenu();
                uis.cursory = -100;
	        UI_PopMenu();
        }
...
void UI_VoteFraglimitMenuInternal( void )
{
	...
	s_votemenu_fraglmit.banner.string        = COM_Localize(297);
	...
	y+=FRAGLIMIT_MENU_VERTICAL_SPACING;
	setMenutext(&s_votemenu_fraglmit.bLimitInf,y,ID_INF,0,COM_Localize(303));
