...
static void UI_DriverInfo_Menu( void )
{
	...
	s_driverinfo.banner.string	  = COM_Localize(153);
...
static const char *builtinResolutions[ ] =
{
	...
	"2048x1536",
	"856x480",
        // Mix3r_Durachok: let's add some extra modes from cl_main.c (engine client)
        "1280x960",
	"1280x720",
	"1280x800",
	"1366x768",
...
static int GraphicsOptions_FindBuiltinResolution( int mode )
{
	...
	for ( i = 2; builtinResolutions[ i ]; i++ ) {
...
static int GraphicsOptions_FindDetectedResolution( int mode )
{
	...
	{
		if( Q_strequal( builtinResolutions[ mode+2 ], detectedResolutions[ i ] ) )
			return i;
	}
...
static void GraphicsOptions_ApplyChanges( void *unused, int notification )
{
	...
	if( resolutionsDetected )
	{
		...
		mode = GraphicsOptions_FindBuiltinResolution( s_graphicsoptions.mode.curvalue-2 );
		if( mode == -1 )
		{
			char w[ 16 ], h[ 16 ];
	...
	{
		trap_Cvar_SetValue( "r_imageUpsample", 2 );
...
static void GraphicsOptions_Event( void* ptr, int event ) {
	...
	case ID_LIST:
		...
		s_graphicsoptions.mode.curvalue        = GraphicsOptions_FindDetectedResolution(ivo->mode)+2;
                s_graphicsoptions.ratio.curvalue       = resToRatio[ s_graphicsoptions.mode.curvalue ];
...
static void GraphicsOptions_TQEvent( void *ptr, int event ) {
	if( event != QM_ACTIVATED ) {
        ...
        trap_Cvar_SetValue( "r_intensity", 1 + (s_graphicsoptions.driver.curvalue * 3) * 0.01f );
        //////wop
...
//APSFIX - rework this
	GraphicsOptions_UpdateMenuItems();
        UI_DrawString( 506, 153, va("%.2f",1 + (s_graphicsoptions.driver.curvalue * 3) * 0.01f), UI_LEFT|UI_SMALLFONT, colorWhite );
...
{
	s_graphicsoptions.mode.curvalue = GraphicsOptions_FindDetectedResolution( trap_Cvar_VariableValue( "r_mode" )+2 );
        s_graphicsoptions.driver.curvalue = (int)(((trap_Cvar_VariableValue("r_intensity")-1.0f) * 1000.0f)/3);
        s_graphicsoptions.driver.curvalue = (int)s_graphicsoptions.driver.curvalue * 0.1;
	...
	if ( trap_Cvar_VariableValue( "r_imageUpsample" ) > 0 ) {
...
void GraphicsOptions_MenuInit( void )
{
	...
	memset( &s_graphicsoptions, 0 ,sizeof(graphicsoptions_t) );
        ...
        enabled_names[0] = COM_Localize(154);
        enabled_names[1] = COM_Localize(155);
        aniso_names[0] = COM_Localize(154);
        quality_names[0] = COM_Localize(145);
        quality_names[1] = COM_Localize(146);
        quality_names[2] = COM_Localize(147);
        filter_names[0] = COM_Localize(156);
        filter_names[1] = COM_Localize(157);
        lighting_names[0] = COM_Localize(158);
        lighting_names[1] = COM_Localize(159);
        s_driver_names[0] = COM_Localize(160);
        tq_names[0] =  COM_Localize(160);
        tq_names[1] =  COM_Localize(161);
        tq_names[2] =  COM_Localize(162);
        s_graphics_options_names[0] = COM_Localize(148);
        s_graphics_options_names[1] = COM_Localize(147);
        s_graphics_options_names[2] = COM_Localize(146);
        s_graphics_options_names[3] = COM_Localize(163);
        s_graphics_options_names[4] = COM_Localize(164);
        s_graphics_options_names[5] = COM_Localize(165);
	...
	s_graphicsoptions.banner.string  	   = COM_Localize(129);
	...
	s_graphicsoptions.graphics.string			= COM_Localize(138);
	...
	s_graphicsoptions.display.string			= COM_Localize(139);
	...
	s_graphicsoptions.sound.string				= COM_Localize(140);
	...
	s_graphicsoptions.network.string			= COM_Localize(141);
	s_graphicsoptions.network.style				= UI_RIGHT;
	s_graphicsoptions.network.color				= color_red;

	y = 240 - 7 * (BIGCHAR_HEIGHT + 2);
	s_graphicsoptions.list.generic.type     = MTYPE_SPINCONTROL;
	s_graphicsoptions.list.generic.name     = COM_Localize(166);
	...
	//s_graphicsoptions.driver.generic.name  = COM_Localize(167);
	...
	//s_graphicsoptions.driver.curvalue      = (uis.glconfig.driverType == GLDRV_VOODOO);
        s_graphicsoptions.driver.generic.type     = MTYPE_SLIDER;
        s_graphicsoptions.driver.generic.name	  = COM_Localize(149); //brightness
	s_graphicsoptions.driver.generic.flags	  = QMF_PULSEIFFOCUS|QMF_SMALLFONT;
	s_graphicsoptions.driver.generic.x	  = 400;
	s_graphicsoptions.driver.generic.y	  = y;
	s_graphicsoptions.driver.minvalue         = 0;
	s_graphicsoptions.driver.maxvalue         = 100;
	s_graphicsoptions.driver.generic.callback = GraphicsOptions_TQEvent;
	...
	s_graphicsoptions.allow_extensions.generic.name	    = COM_Localize(168);
	...
	y += BIGCHAR_HEIGHT+2;
        ...
        s_graphicsoptions.ratio.generic.name     = COM_Localize(169);
        ...
        resolutions[0] = COM_Localize(329);
        resolutions[1] = COM_Localize(165);

	// references/modifies "r_mode"
	s_graphicsoptions.mode.generic.type     = MTYPE_SPINCONTROL;
	s_graphicsoptions.mode.generic.name     = COM_Localize(170);
	...
	s_graphicsoptions.fs.generic.name	  = COM_Localize(171);
	...
	s_graphicsoptions.lighting.generic.name	 = COM_Localize(172);
	...
	s_graphicsoptions.flares.generic.name	  = COM_Localize(173);
	...
	s_graphicsoptions.bloom.generic.name	  = COM_Localize(174);
	...
	s_graphicsoptions.drawfps.generic.name	 = COM_Localize(175);
	...
	s_graphicsoptions.geometry.generic.name	 = COM_Localize(176);
	...
	s_graphicsoptions.tq.generic.name	= COM_Localize(177);
	...
	s_graphicsoptions.texturebits.generic.name	= COM_Localize(178);
	...
	s_graphicsoptions.filter.generic.name	= COM_Localize(179);
	...
	s_graphicsoptions.aniso.generic.name	= COM_Localize(180);
	...
	s_graphicsoptions.driverinfo.string           = COM_Localize(153);
