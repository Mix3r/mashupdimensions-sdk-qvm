...
static ui_menuitem_t s_items[] = {
	{ID_NEXTMAP, 292, &s_votemenu_logic.nextmap},
	{ID_RESTART, 211, &s_votemenu_logic.map_restart},
	{ID_MAP,293, &s_votemenu_logic.map},
	{ID_GAMETYPE,294, &s_votemenu_logic.gametype},
	{ID_KICK,295, &s_votemenu_logic.clientkick},
	{ID_DOWARMUP,296, &s_votemenu_logic.g_doWarmup},
	{ID_FRAG,297, &s_votemenu_logic.fraglimit},
	{ID_TIME,298, &s_votemenu_logic.timelimit},
	{ID_CUSTOM,299, &s_votemenu_logic.custom},
	{ID_YES,300, &alwaysTrue},
	{ID_NO,301, &alwaysTrue}
};
...
void UI_VoteMenuMenuInternal( void )
{
	...
	s_votemenu.banner.string		  = COM_Localize(209);
	...
	for (i = 0; i < ARRAY_LEN(s_votemenu.bButtons); ++i) {
		...
		s_votemenu.bButtons[i].string              = COM_Localize(s_items[i].string);
	...
	if(s_votemenu_logic.g_doWarmupEnabled) {
		for (i = 0; i < ARRAY_LEN(s_votemenu.bButtons); ++i) {
			if (s_votemenu.bButtons[i].generic.id == ID_DOWARMUP) {
				s_votemenu.bButtons[i].string = COM_Localize(302);
			}
		}
	}
