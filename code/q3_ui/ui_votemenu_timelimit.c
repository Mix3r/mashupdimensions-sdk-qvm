...
static void VoteMenu_Timelimit_Event( void* ptr, int event )
{
	...
	switch (((menucommon_s*)ptr)->id)
	{
        ...
        if (chosen_lim >= 0) {
                trap_Cmd_ExecuteText( EXEC_APPEND, va("callvote timelimit %i",chosen_lim));
                UI_PopMenu();
                uis.cursory = -100;
	        UI_PopMenu();
        }
...
void UI_VoteTimelimitMenuInternal( void )
{
	...
	s_votemenu_Timelmit.banner.string        = COM_Localize(298);
	s_votemenu_Timelmit.banner.color         = color_white;
	s_votemenu_Timelmit.banner.style         = UI_CENTER;

	y = 98;
	setTimeMenutext(&s_votemenu_Timelmit.bLimit10,y,ID_10,10,"10 M");
	y+=TimeLIMIT_MENU_VERTICAL_SPACING;
	setTimeMenutext(&s_votemenu_Timelmit.bLimit15,y,ID_15,15,"15 M");
	y+=TimeLIMIT_MENU_VERTICAL_SPACING;
	setTimeMenutext(&s_votemenu_Timelmit.bLimit20,y,ID_20,20,"20 M");
	y+=TimeLIMIT_MENU_VERTICAL_SPACING;
	setTimeMenutext(&s_votemenu_Timelmit.bLimit30,y,ID_30,30,"30 M");
	y+=TimeLIMIT_MENU_VERTICAL_SPACING;
	setTimeMenutext(&s_votemenu_Timelmit.bLimit40,y,ID_40,40,"40 M");
	y+=TimeLIMIT_MENU_VERTICAL_SPACING;
	setTimeMenutext(&s_votemenu_Timelmit.bLimit50,y,ID_50,50,"50 M");
	y+=TimeLIMIT_MENU_VERTICAL_SPACING;
	setTimeMenutext(&s_votemenu_Timelmit.bLimitInf,y,ID_INF,0,COM_Localize(303));
