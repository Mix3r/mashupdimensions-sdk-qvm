...
char	*ui_medalSounds[] = {
	...
	"sound/feedback/perfect.wav",
        "sound/feedback/antihop.wav",
};

// Mix3r_Durachok: load respective language version of sound
sfxHandle_t UI_CacheLocalizedSound(char *snd) {
        char *trysnd;
        ...
        for( n = 1; n < 3; n++ ) {
                ...
                rethandle = trap_S_RegisterSound(trysnd, qfalse);
                if (rethandle) {
                        break;
                }
        }
        ...
        if ((level == 0) || (levelSet > (level-1)/ARENAS_PER_TIER) || (postgameMenuInfo.won == postgameMenuInfo.lastTier)) {
			...
			if( postgameMenuInfo.won == 1) {
				...
				return;
                        ...
                        arenaInfo = UI_GetArenaInfoByNumber( postgameMenuInfo.level );
                        if (arenaInfo) {
                                sf = Info_ValueForKey( arenaInfo, "special" );
                                if ( sf[0] ) {
                                        if ( Q_strequal( sf, "Final" )) {
                                                level = -1;
                                        }
                                }
                        }
                        ...
                        if (arenaInfo) {
                                ...
                                sf = Info_ValueForKey( arenaInfo, "special" );
                                if ( sf[0] ) {
                                        if ( Q_strequal( sf, "Training" ) && level > 1) {
                                                arenaInfo = UI_GetSpecialArenaInfo( "Final" );
                                        }
                                }
                                sf = va("video/tier%i.RoQ",postgameMenuInfo.won + 1);
                                Q_strncpyz(postgameMenuInfo.placeNames[0], sf, sizeof(postgameMenuInfo.placeNames[0]));
                                sf = Info_ValueForKey( arenaInfo, "map" );
                        } else {
                                trap_Cvar_SetValue( "ui_spSelection", -ARENAS_PER_TIER );
                                level = postgameMenuInfo.level;
                                Q_strncpyz(postgameMenuInfo.placeNames[0], "video/end.RoQ", sizeof(postgameMenuInfo.placeNames[0]));
                                currentLevel = trap_FS_FOpenFile(postgameMenuInfo.placeNames[0], &f_seqpic, FS_READ);
                                if (currentLevel > 0) {
                                        trap_FS_FCloseFile(f_seqpic);
                                        sf = "end";
                                } else {
                                        sf = "oa_end";
                                }
                        ...
                        sf = va("video/%s_1.jpg",sf);
                        currentLevel = trap_FS_FOpenFile(sf, &f_seqpic, FS_READ);
                        if (currentLevel > 0) {
                                trap_FS_FCloseFile(f_seqpic);
                        } else {
                                currentLevel = trap_FS_FOpenFile(postgameMenuInfo.placeNames[0], &f_seqpic, FS_READ);
...
static int medalLocations[7] = {144, 448, 88, 504, 32, 560, 530};
...
static void UI_SPPostgameMenu_DrawAwardsPresentation( int timer ) {
	...
	UI_DrawProportionalString( 320, 64, COM_Localize(postgameMenuInfo.awardsEarned[awardNum]+312), UI_CENTER, color );
	...
	if( !postgameMenuInfo.playedSound[awardNum] ) {
		postgameMenuInfo.playedSound[awardNum] = qtrue;
                vo_award = UI_CacheLocalizedSound( ui_medalSounds[postgameMenuInfo.awardsEarned[awardNum]]);
                if (vo_award) {
		        trap_S_StartLocalSound( vo_award, CHAN_ANNOUNCER );
                }
	}
}
...
void UI_SPPostgameMenu_Cache( void ) {
	...
	for( n = 0; n < 6; n++ ) {
        ...
        for( n = 0; n < 5; n++ ) {
		trap_R_RegisterShaderNoMip( ui_skillPics[n] );
	}
	if( buildscript ) {
...
void UI_SPPostgameMenu_f( void ) {
	...
	if ( playerGameRank == 1 || postgameMenuInfo.skillsp == 10000) {
                playerGameRank = 1;
	...
	postgameMenuInfo.scoreboardtime = uis.realtime;
        postgameMenuInfo.skillsp = Com_Clamp( 1, 5, trap_Cvar_VariableValue( "g_spSkill" ) );
	...
	if ( playerGameRank != 1 ) {
		...
		postgameMenuInfo.winnerSound = UI_CacheLocalizedSound( "sound/player/announce/youwin.wav");
